...
const frontend = {
        ...
        $(function () {
            ...
            const banner = {
                ...
                declineAllButton: function () {
                    ...
                    CCFW.storage.bannerHidden.set(true)
                    utilities.hideBanner()
                    frontend.clear(CCFW.storage.allowed.get())
                    window.location.reload(false)
                    return false
                },
    ...
    clear: (allowList) => {
        ...
        if (!allowList.includes('ua')) {
            //Google Analytics
            frontend.kill.related('_ga')
            frontend.kill.related('_ga_')
            frontend.kill.cookies('_gid')
            frontend.kill.related('_gat')
        }

        if (!allowList.includes('html')) {
            //Facebook cookies
            frontend.kill.cookies('fr')
            frontend.kill.cookies('tr')
            frontend.kill.cookies('_fbc')
            frontend.kill.cookies('_fbp')
            frontend.kill.cookies('PSUK_source')
        }

        if (!allowList.includes('gclidw')) {
            //Google conversion linker cookies
            frontend.kill.cookies('_gcl_au')
            frontend.kill.cookies('_gcl_dc')
            frontend.kill.cookies('_gcl_aw')
        }
    },
    kill: {
        cookies: (name) => {
        ...
        related: (name) => {
            ...
            frontend.kill.cookies(name)
            const cookies = document.cookie.split(';') // array of cookies
            let ii = 0;

            for (ii; ii < cookies.length; ii++) {
                let cookie = cookies[ii].trim()
                ...
                let full_name = eqPos > -1 ? cookie.substring(0, eqPos) : cookie
                if (full_name.substring(0, name.length) === name) {
                    frontend.kill.cookies(full_name)
                }
            }
        }
    }

 ...
 * 
 * This file follows https://github.com/pantheon-systems/wp-redis
 * Modifications have been annotated with `// *` & `// ^`,
 * - `*` indicates a change has been up-streamed.
 * - `^` indicates a change that is specific to our implementation, and not up-streamed.
 */
...
class WP_Object_Cache {
	...
	public function get( $key, $group = 'default', $force = false, &$found = null ) {
		...
		$value = is_numeric( $value ) ? intval( $value ) : igbinary_unserialize( $value );
	...
	public function stats() {
		...
		foreach ( $this->cache as $group => $cache ) {
			$out[] = '<li><strong>Group:</strong> ' . esc_html( $group ) . ' - ( ' . number_format( strlen( igbinary_serialize( $cache ) ) / 1024, 2 ) . 'k )</li>';
		}
		$out[] = '</ul>';
	...
	public function build_client_parameters( $redis_server ) {
		...
		$database = WP_REDIS_USE_RELAY ? 4 : 3;
		// *
